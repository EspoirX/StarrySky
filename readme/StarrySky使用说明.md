...
实现拦截器需要继承 StarrySkyInterceptor ，然后实现 process 和 getTag 方法。

```kotlin
class ATestInterceptor : StarrySkyInterceptor() {
    override fun process(songInfo: SongInfo?, callback: InterceptCallback) {
        //do something...
        callback.onNext(songInfo)
        callback.onInterrupt("拦截器中断拉...")
    }
...
```
InterceptCallback 回调有两个方法，分别是 onNext 和 onInterrupt，onNext 的意思是往下走，onInterrupt 是拦截。
...
拦截器默认是运行在 UI 线程的，如果想要让拦截器运行在子线程中，可以通过 addInterceptor 方法的第二个参数，传入 InterceptorThread.IO 即可，
如果传入 InterceptorThread.UI，则代表运行在 UI 线程。
