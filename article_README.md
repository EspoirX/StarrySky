...
可以看到，`PlayControl`其实是一个`Binder`,连接这客户端和服务端。  
...
它实现了 Playback.Callback 接口，而 Playback 定义了播放器相关的类。  
...
###### 通知栏的分类
NotificationCompat.Builder 里面 `ContentView` 的方法一共有两个，一个是 `setCustomContentView()`  
...
在配置通知栏的时候，最重要的就是如何获取到资源文件和布局里面相关的控件，是通过 `Resources#getIdentifier` 方法去获取：
```java
